...
export function GameBoard({ roomId, playerId, onBackToLobby }: GameBoardProps) {
  ...
  const fetchGameData = async () => {
    ...
    if (guessesData) {
      setGuesses(guessesData.map(g => ({ ...g, player: g.players })))
    }
  }
  ...
  const submitGuess = async () => {
    ...
    if (isCorrect) {
      ...
      await supabase
        .from('players')
        .update({ score: supabase.rpc('increment_score', { amount: 10 }) })
        .eq('id', playerId)
  ...
  const handleRoundEnd = async () => {
    ...
    if (nextRound <= room.max_rounds) {
      // Start next round
      const newWord = DRAWING_WORDS[Math.floor(Math.random() * DRAWING_WORDS.length)]
  ...
  if (!room) {
    return <div className="min-h-screen bg-gradient-to-br from-purple-400 via-pink-300 to-yellow-300 flex items-center justify-center">
